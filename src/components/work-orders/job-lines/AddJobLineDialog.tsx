...
export function AddJobLineDialog({
  ...
  return <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button size="sm" className="flex items-center gap-2 text-lg text-[#050d05] bg-[#07f007] rounded-lg">
          <Plus className="h-4 w-4" />
          Add Job Line
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto border shadow-lg rounded-md bg-white">
        <DialogHeader className="bg-background border-b pb-4">
          <DialogTitle>Add Services to Work Order</DialogTitle>
        </DialogHeader>
        <div className="bg-background p-6">
          <ServiceBasedJobLineForm workOrderId={workOrderId} onSubmit={handleJobLineAdd} onCancel={() => setIsOpen(false)} />
        </div>
      </DialogContent>
    </Dialog>;
}