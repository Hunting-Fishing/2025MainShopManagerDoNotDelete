...
interface AddJobLineDialogProps {
  ...
  onJobLineAdd?: (newJobLinesData: Omit<WorkOrderJobLine, 'id' | 'createdAt' | 'updatedAt'>[]) => void;
}
...
export function AddJobLineDialog({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (onJobLineAdd) {
      // For EditableJobLinesGrid usage
      const newJobLineData: Omit<WorkOrderJobLine, 'id' | 'createdAt' | 'updatedAt'> = {
      ...
      onJobLineAdd([newJobLineData]);
    } else if (onAdd) {
      // For JobLinesGrid usage
  ...
  const DialogComponent = controlledOpen !== undefined ? Dialog : DialogTrigger;

  if (controlledOpen !== undefined) {
    // Controlled mode (used by JobLinesGrid)
    return (
  ...
  // Uncontrolled mode with trigger button (used by EditableJobLinesGrid)
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
