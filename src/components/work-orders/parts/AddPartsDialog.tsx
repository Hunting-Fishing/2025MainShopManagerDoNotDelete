...
interface AddPartsDialogProps {
  workOrderId: string;
  jobLineId?: string;
  onPartsAdd: (parts: WorkOrderPartFormValues[]) => void;
  open?: boolean;
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-sky-300">
        ...
        <Tabs defaultValue="inventory" className="w-full">
          ...
          <TabsContent value="inventory" className="space-y-4">
            <InventoryPartsTab workOrderId={workOrderId} jobLineId={jobLineId} onAddPart={handleAddPart} />
          </TabsContent>
          
          <TabsContent value="non-inventory" className="space-y-4">
            <NonInventoryPartsTab workOrderId={workOrderId} jobLineId={jobLineId} onAddPart={handleAddPart} />
          </TabsContent>
        </Tabs>

        {selectedParts.length > 0 && <div className="border-t pt-4">
            <h4 className="font-medium mb-2">Selected Parts ({selectedParts.length})</h4>
            <div className="space-y-2 max-h-32 overflow-y-auto">
              {selectedParts.map((part, index) => <div key={index} className="flex items-center justify-between p-2 border rounded">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => handleRemovePart(index)} className="text-red-600 hover:text-red-700">
                    Remove
                  </Button>
                </div>)}
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <Button variant="outline" onClick={() => setSelectedParts([])}>
                Clear All
              </Button>
